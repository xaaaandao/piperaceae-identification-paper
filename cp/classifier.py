...
def my_ensemble_classifier(cfg, dataset, list_result_classifier, n_features, n_samples, n_patch=None, orientation=None):
    ...
    save(None, cfg, classifier_name, dataset, list_result_fold, list_time, n_features, path_classifier)
...
def ensemble_classifier(cfg, dataset, index, list_best_classifiers, n_features, n_samples, path, x, y, n_patch=None,
    ...
    save(None, cfg, classifier_name, dataset, list_result_fold, list_time, n_features, path)


def classification_data(cfg, color_mode, dataset, dim, file_input, index, n_features, n_samples, path, x, x_surf, y, n_patch=None,
                        orientation=None):
    list_classifiers = [
        ...
        sklearn.svm.SVC(random_state=cfg["random_state"], probability=True, cache_size=4096)
    ]

    list_best_classifiers = []
    list_result_classifier = []

    for classifier in list_classifiers:
        classifier_name = classifier.__class__.__name__

        model = sklearn.model_selection.GridSearchCV(estimator=classifier, param_grid=hyperparams[classifier_name], scoring="accuracy",
                                                     cv=5, n_jobs=-1, pre_dispatch="4*n_jobs")
        ...
        data = [file_input, x.shape[1], n_samples, n_patch, orientation, color_mode, dim]
        columns = ["file_input", "n_features", "n_samples", "n_patch", "orientation", "color_mode", "dim"]
        ...
        break
        save(best_params, cfg, classifier_name, dataset, list_result_fold, list_time, n_features, path_completed)
