#!/bin/bash
TAXON=specific_epithet
METRIC=f1_weighted
DIR_FEATURES=../dataset_gimp/imagens_br/features
DIR_LABEL=../dataset_gimp/imagens_br/imagens

for img_size in 512; do
	for cnn in vgg16; do
		for color in RGB;	do
			for patch in 3; do
				for threshold in 5; do
					echo ${cnn} ${img_size} ${threshold} ${color} ${METRIC}
					python main.py -i ${DIR_FEATURES}/${color}/segmented_unet/${img_size}/patch=${patch}/${TAXON}/${threshold}/${cnn}/horizontal -l ${DIR_LABEL}/${color}/${TAXON}/${img_size}/${threshold}/label2.txt -m ${METRIC} --pca
				done
			done
		done
	done
done

# for img_size in 256 400 512; do
# 	for file in lbp surf64; do
# 		for threshold in 5 10 20; do
# 			color=grayscale
# 			segmented=unet
# 			python main.py -i ${DIR_FEATURES}/${color}/segmented_${segmented}/${img_size}/patch=1/${TAXON}/${threshold}/${file}.txt -l ${DIR_LABEL}/${color}/segmented_${segmented}/${img_size}/${threshold}/label2.txt -m ${METRIC} --pca
# 		done
# 	done
# done