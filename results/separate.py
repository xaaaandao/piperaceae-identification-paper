...
path = '/media/xandao/c2f58d30-ff2c-47f7-95af-91ad6fd69760/resultados/regioes/new'
for regioes in ['Norte', 'Sul', 'Sudeste', 'Nordeste', 'Centro-Oeste']:
    list_cmd = []
    for p in pathlib.Path(os.path.join(path)).rglob('info.csv'):
        # pathlib.Path(regioes).mkdir(exist_ok=True, parents=True)
