...
from save.save_top_k import mean_top_k
...
def save_mean(list_result_fold, list_time, path):
    ...
    list_mean_accuracy = get_list_of_results_all_rule_by_metric(list_result_fold, 'accuracy')
    list_mean_f1 = get_list_of_results_all_rule_by_metric(list_result_fold, 'f1_score')

    accuracy = get_mean_metric(list_mean_accuracy)
    f1 = get_mean_metric(list_mean_f1)
    mean_top_k(list_result_fold, path)

    values = get_values_sheet_mean(accuracy, f1, mean_time_search_best_params, mean_time_train_valid,
                                   std_time_search_best_params, std_time_train_valid)
...
def get_values_sheet_mean(accuracy, f1, mean_time_search_best_params, mean_time_train_valid,
                          std_time_search_best_params, std_time_train_valid):
...
def get_mean_metric(list_mean_metric):
    mean_metric_max, std_metric_max = get_mean_and_std_by_metric_and_rule(list_mean_metric, 'max')
    mean_metric_sum, std_metric_sum = get_mean_and_std_by_metric_and_rule(list_mean_metric, 'sum')
    mean_metric_prod, std_metric_prod = get_mean_and_std_by_metric_and_rule(list_mean_metric, 'prod')
...
def get_mean_and_std_by_metric_and_rule(list_mean_metric, rule):
...
def get_list_of_results_all_rule_by_metric(list_result_fold, metric):
    return [calculate_mean_and_std_by_metric_and_rule(list_result_fold, metric, 'sum'),
            calculate_mean_and_std_by_metric_and_rule(list_result_fold, metric, 'max'),
            calculate_mean_and_std_by_metric_and_rule(list_result_fold, metric, 'prod')]


def calculate_mean_and_std_by_metric_and_rule(list_result_fold, metric, rule):
