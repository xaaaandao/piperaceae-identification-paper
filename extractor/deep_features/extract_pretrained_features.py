...
def extract_features(model_name, patches, folds, spec_height, spec_width, 
	...
	for n_patches in patches:
		...
		input_shape = (math.floor(spec_height/n_patches), math.floor(spec_width/n_patches), 3)
		...
		for fold in folds:
			...
			for fname in sorted(glob.glob(input_path_proto % (fold))):
				...
				for p in next_patch(spec, n_patches):
					j=0
					for p2 in next_patch2(p, n_patches):
						p2 = preprocess_input(p2)
						# img_p = image.array_to_img(spec)
						p2 = np.expand_dims(p2, axis=0)
						# image.save_img(f'a{i}-{j}.png', img_p)
						features.append(model.predict(p2))
					# j+=1
					i+=1
				# exit(1)
