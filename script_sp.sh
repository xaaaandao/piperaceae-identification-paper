		...
		for cnn in mobilenetv2 resnet50v2 vgg16; do
			for color in RGB grayscale; do
				for patch in 1 3; do
					...
					python main.py -i ${DIR_FEATURES}/${color}/segmented_${segmented}/${img_size}/patch=${patch}/${cnn}/horizontal -l ${DIR_LABEL}/${color}/segmented_${segmented}/${img_size}/label2.txt -m ${METRIC} --pca
				done
			done
		done
