...
def run_all_classifiers(cfg, current_datetime, data, handcraft, list_labels, metric):
    for classifier in list_classifiers:
        ...
        run_folds(best, classifier_name, data, handcraft, list_labels, list_result_fold, list_time, path, split,
                  time_find_best_params)
        save(best['params'], cfg, data, list_labels, list_result_fold, list_time, metric, path)


def run_folds(best, classifier_name, data, handcraft, list_labels, list_result_fold, list_time, path, split,
              time_find_best_params):
    for fold, (index_train, index_test) in enumerate(split):
        ...
        result_max_rule, result_prod_rule, result_sum_rule = get_result(data, fold, list_labels, y_pred, y_test,
                                                                        handcraft=handcraft)
